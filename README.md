# Chaitanay Kapoor
**Data Analyst | Full Stack Developer | AWS Cloud Practitioner**

📧 chaitanay.kapoor1206@gmail.com | 📱 +91 8700062395 | [LinkedIn](https://linkedin.com/in/chaitanay-kapoor)

---

## 📊 Striver SDE Sheet Analytics

<div align="center">

### 🏆 DSA Progress Dashboard
**Profile:** [chaitanay04 on TakeUForward](https://takeuforward.org/profile/chaitanay04)

![Coding Stats](https://img.shields.io/badge/SDE%20Sheet-TakeUForward-orange?style=for-the-badge&logo=leetcode&logoColor=white)
![Problem Solving](https://img.shields.io/badge/DSA-Problem%20Solving-blue?style=for-the-badge&logo=algorithm&logoColor=white)
![Data Structures](https://img.shields.io/badge/Focus-Data%20Structures-green?style=for-the-badge&logo=tree&logoColor=white)

### 📈 Animated Progress Tracker

```
🔥 STRIVER SDE SHEET PROGRESS 🔥

Arrays & Strings     ████████░░ 80% (24/30)
Linked Lists         ██████░░░░ 60% (18/30)  
Binary Trees         ███████░░░ 70% (21/30)
Dynamic Programming  █████░░░░░ 50% (15/30)
Graphs & BFS/DFS     ████░░░░░░ 40% (12/30)
Recursion & Backtrack ██████████ 90% (27/30)

📊 Total Solved: 117/180 Questions
🎯 Completion Rate: 65%
⚡ Current Streak: Building...
```

---

![GitHub Stats](https://github-readme-stats.vercel.app/api?username=chaitanay2004&show_icons=true&theme=tokyonight&hide_border=true&include_all_commits=true&count_private=true)

![GitHub Activity](https://github-readme-activity-graph.vercel.app/graph?username=chaitanay2004&theme=tokyo-night&hide_border=true&custom_title=GitHub%20Contribution%20Activity)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=chaitanay2004&layout=compact&theme=tokyonight&hide_border=true)

</div>

## 📈 Repository Analytics

<div align="center">

### 🗂️ Full Stack Projects
![Task Manager](https://github-readme-stats.vercel.app/api/pin/?username=chaitanay2004&repo=Task-Manager&theme=tokyonight&hide_border=true)

### 🎮 Interactive Games
![Simon Game](https://github-readme-stats.vercel.app/api/pin/?username=chaitanay2004&repo=Simon_game&theme=tokyonight&hide_border=true)
![Drum Kit](https://github-readme-stats.vercel.app/api/pin/?username=chaitanay2004&repo=Drum_kit&theme=tokyonight&hide_border=true)
![Dice Game](https://github-readme-stats.vercel.app/api/pin/?username=chaitanay2004&repo=Dice_game&theme=tokyonight&hide_border=true)

### 🛒 Web Development
![Amazon Clone](https://github-readme-stats.vercel.app/api/pin/?username=chaitanay2004&repo=Amazon_clone&theme=tokyonight&hide_border=true)

</div>

## 🛠️ Technology Stack Analytics

<div align="center">

<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/javascript/javascript-original.svg" alt="JavaScript" width="50" height="50"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/html5/html5-original.svg" alt="HTML5" width="50" height="50"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/css3/css3-original.svg" alt="CSS3" width="50" height="50"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/nodejs/nodejs-original.svg" alt="Node.js" width="50" height="50"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/express/express-original.svg" alt="Express.js" width="50" height="50"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/mongodb/mongodb-original.svg" alt="MongoDB" width="50" height="50"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/mysql/mysql-original.svg" alt="MySQL" width="50" height="50"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/java/java-original.svg" alt="Java" width="50" height="50"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/c/c-original.svg" alt="C" width="50" height="50"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/cplusplus/cplusplus-original.svg" alt="C++" width="50" height="50"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/amazonwebservices/amazonwebservices-original.svg" alt="AWS" width="50" height="50"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/git/git-original.svg" alt="Git" width="50" height="50"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/vscode/vscode-original.svg" alt="VS Code" width="50" height="50"/>

</div>

## 🚀 Project Portfolio

### 🗂️ Task Manager System
**Repository:** [Task-Manager](https://github.com/chaitanay2004/Task-Manager) | **Language:** JavaScript  
**Stack:** Node.js, Express.js, MongoDB, Cloudinary  
- 🏗️ Architected for 50+ concurrent users with distinct User and Admin roles
- 🔐 Implemented JWT-based authentication and role-based access control  
- ☁️ Integrated Cloudinary for secure file uploads (100+ files handled)
- ⚡ Optimized API design reducing response latency by 30%

### 🎮 Interactive Game Collection

#### Simon Game
**Repository:** [Simon_game](https://github.com/chaitanay2004/Simon_game) | **Language:** JavaScript  
- 🎨 Interactive UI with 95% cross-browser compatibility
- ⚡ Enhanced user interaction time by 40% with smooth animations
- 📱 Responsive design across 10+ device resolutions

#### Drum Kit
**Repository:** [Drum_kit](https://github.com/chaitanay2004/Drum_kit) | **Language:** JavaScript  
- 🥁 Interactive musical instrument simulation
- 🎵 Real-time audio feedback and visual effects

#### Dice Game
**Repository:** [Dice_game](https://github.com/chaitanay2004/Dice_game) | **Language:** HTML  
- 🎲 Interactive dice rolling game with dynamic UI
- 🎯 Real-time game logic and winner detection

### 🛒 Amazon Clone
**Repository:** [Amazon_clone](https://github.com/chaitanay2004/Amazon_clone) | **Language:** HTML  
- 🛍️ E-commerce interface replication
- 📱 Responsive design with modern CSS techniques

## 💼 Professional Experience Analytics

### 📊 Data Analyst Intern | Lagozon Technologies Pvt. Ltd.
**Duration:** June 2025 - July 2025

<div align="center">

| Metric | Value | Impact |
|--------|-------|--------|
| **SKUs Analyzed** | 300+ | Product performance insights |
| **Data Entries Processed** | 1,000+ | Time-series analysis capability |
| **Decision-Making Improvement** | 35% | Through Power BI dashboards |
| **Processing Time Reduction** | 40% | Via automated report generation |
| **Sales Contributors Identified** | 60% | For inventory optimization |
| **Forecasting Accuracy** | +20% | Enhanced demand prediction |

</div>

## 🎓 Education & Certifications Analytics

<div align="center">

| Institution | Degree | CGPA | Status |
|-------------|---------|------|--------|
| **Vellore Institute of Technology** | B.Tech Information Technology | 8.25/10 | Expected Aug 2026 |

| Certification | Provider | Status |
|---------------|----------|--------|
| **AWS Cloud Practitioner** | Amazon Web Services | ✅ Certified |

</div>

## 👑 Leadership Analytics

### Vice-Chairperson | Mozilla Firefox Club
**Tenure:** January 2024 - March 2025  
- 🦊 Led community initiatives and technical workshops
- 📈 Managed club operations and member engagement

## 📊 Contribution Activity

<div align="center">

![Activity Graph](https://github-readme-activity-graph.vercel.app/graph?username=chaitanay2004&theme=tokyo-night&hide_border=true)

</div>

## 🏆 Achievement Metrics

<div align="center">

[![trophy](https://github-profile-trophy.vercel.app/?username=chaitanay2004&theme=tokyonight&no-frame=true&row=1&column=7)](https://github.com/ryo-ma/github-profile-trophy)

</div>

## 📱 Profile Analytics

<div align="center">

![Profile Views](https://komarev.com/ghpvc/?username=chaitanay2004&color=blueviolet&style=for-the-badge&label=Profile+Views)

</div>

---

<div align="center">

**💡 Currently expanding skills in Cloud Architecture and Machine Learning for Data Analysis**

**🔗 Connect with me for collaboration opportunities**

</div>
